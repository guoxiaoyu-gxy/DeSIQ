...
paths = {}
paths["QA_BERT_multiplechoice"] = "./socialiq/SOCIAL-IQ_QA_BERT_MULTIPLE_CHOICE.csd"
...
def calc_accuracy(correct, incorrect_1, incorrect_2, incorrect_3):
    correct_ = correct.cpu()
    incorrect_1_ = incorrect_1.cpu()
    incorrect_2_ = incorrect_2.cpu()
    incorrect_3_ = incorrect_3.cpu()
    return numpy.array((correct_ > incorrect_1_) * (correct_ > incorrect_2_) * (correct_ > incorrect_3_),
                       dtype="float32").sum() / correct.shape[0]
...
def build_qa_binary(qa_glove, keys):
    return qai_to_tensor(qa_glove, keys, 3)
...
def process_data(keys):
    qa_glove = social_iq["QA_BERT_multiplechoice"]
...
def feed_forward(keys, t5_model, fc, q_lstm, a_lstm, v_lstm, t_lstm, ac_lstm, mfn_mem, mfn_delta1, mfn_delta2, mfn_tfn,
    ...
    if perturbed_data is not None:
        q, a, i_r = [data[keys[0]:keys[1]] for data in preloaded_data[0]]
        if offset is not None and offset > 0:
            _, i, _ = [data[keys[0]:keys[1]] for data in perturbed_data[0]]
            i = numpy.concatenate((i[:, :, 0, :, :, :], i[:, :, 1, :, :, :],
                                   i[:, :, 2, :, :, :], i[:, :, 0, :, :, :],
                                   i[:, :, 1, :, :, :], i[:, :, 2, :, :, :],
                                   i[:, :, 0, :, :, :], i[:, :, 1, :, :, :],
                                   i[:, :, 2, :, :, :], i[:, :, 0, :, :, :],
                                   i[:, :, 1, :, :, :], i[:, :, 2, :, :, :]),
                                  axis=2)
            i = numpy.expand_dims(i, axis=3).reshape(i_r.shape)
        elif offset is not None and offset == 0:
            a_shuffle = numpy.concatenate((a[:, 1:, :, :, :, :],
                                           numpy.expand_dims(a[:, 0, :, :, :, :], axis=1)), axis=1)
            # print(a_shuffle.shape)
            i = numpy.concatenate((a_shuffle[:, :, 0, :, :, :], a_shuffle[:, :, 1, :, :, :],
                                   a_shuffle[:, :, 2, :, :, :], a_shuffle[:, :, 0, :, :, :],
                                   a_shuffle[:, :, 1, :, :, :], a_shuffle[:, :, 2, :, :, :],
                                   a_shuffle[:, :, 0, :, :, :], a_shuffle[:, :, 1, :, :, :],
                                   a_shuffle[:, :, 2, :, :, :], a_shuffle[:, :, 0, :, :, :],
                                   a_shuffle[:, :, 1, :, :, :], a_shuffle[:, :, 2, :, :, :]),
                                  axis=2).reshape(i_r.shape)
        # print(i.shape)
    else:
    ...
    if context_matters:
        if load_data == 'a':
            ...
            i_h1 = t5_model(inputs_embeds=fc(i_e[:, 0, :, :])).last_hidden_state.transpose(0, 1)
            i_h2 = t5_model(inputs_embeds=fc(i_e[:, 1, :, :])).last_hidden_state.transpose(0, 1)
            i_h3 = t5_model(inputs_embeds=fc(i_e[:, 2, :, :])).last_hidden_state.transpose(0, 1)
        elif load_data == 'qa':
            a_h = t5_model(inputs_embeds=fc(torch.cat((q_e, a_e), dim=1))).last_hidden_state.transpose(0, 1)
            i_h1 = t5_model(inputs_embeds=fc(torch.cat((q_e, i_e[:, 0, :, :]),
                                                       dim=1))).last_hidden_state.transpose(0, 1)
            i_h2 = t5_model(inputs_embeds=fc(torch.cat((q_e, i_e[:, 1, :, :]),
                                                       dim=1))).last_hidden_state.transpose(0, 1)
            i_h3 = t5_model(inputs_embeds=fc(torch.cat((q_e, i_e[:, 2, :, :]),
                                                       dim=1))).last_hidden_state.transpose(0, 1)
        elif load_data == 'qat':
            a_h = t5_model(inputs_embeds=fc(torch.cat((q_e, a_e, t_e), dim=1))).last_hidden_state.transpose(0, 1)
            i_h1 = t5_model(inputs_embeds=fc(torch.cat((q_e, i_e[:, 0, :, :], t_e),
                                                       dim=1))).last_hidden_state.transpose(0, 1)
            i_h2 = t5_model(inputs_embeds=fc(torch.cat((q_e, i_e[:, 1, :, :], t_e),
                                                       dim=1))).last_hidden_state.transpose(0, 1)
            i_h3 = t5_model(inputs_embeds=fc(torch.cat((q_e, i_e[:, 2, :, :], t_e),
                                                       dim=1))).last_hidden_state.transpose(0, 1)
        elif load_data == 'qav':
            a_h = t5_model(inputs_embeds=torch.cat((fc(torch.cat((q_e, a_e), dim=1)), v_fc(v_e)),
                                                   dim=1)).last_hidden_state.transpose(0, 1)
            i_h1 = t5_model(inputs_embeds=torch.cat((fc(torch.cat((q_e, i_e[:, 0, :, :]), dim=1)), v_fc(v_e)),
                                                    dim=1)).last_hidden_state.transpose(0, 1)
            i_h2 = t5_model(inputs_embeds=torch.cat((fc(torch.cat((q_e, i_e[:, 1, :, :]), dim=1)), v_fc(v_e)),
                                                    dim=1)).last_hidden_state.transpose(0, 1)
            i_h3 = t5_model(inputs_embeds=torch.cat((fc(torch.cat((q_e, i_e[:, 2, :, :]), dim=1)), v_fc(v_e)),
                                                    dim=1)).last_hidden_state.transpose(0, 1)
        elif load_data == 'qatv':
            a_h = t5_model(inputs_embeds=torch.cat((fc(torch.cat((q_e, a_e, t_e), dim=1)), v_fc(v_e)),
                                                   dim=1)).last_hidden_state.transpose(0, 1)
            i_h1 = t5_model(inputs_embeds=torch.cat((fc(torch.cat((q_e, i_e[:, 0, :, :], t_e), dim=1)), v_fc(v_e)),
                                                    dim=1)).last_hidden_state.transpose(0, 1)
            i_h2 = t5_model(inputs_embeds=torch.cat((fc(torch.cat((q_e, i_e[:, 1, :, :], t_e), dim=1)), v_fc(v_e)),
                                                    dim=1)).last_hidden_state.transpose(0, 1)
            i_h3 = t5_model(inputs_embeds=torch.cat((fc(torch.cat((q_e, i_e[:, 2, :, :], t_e), dim=1)), v_fc(v_e)),
                                                    dim=1)).last_hidden_state.transpose(0, 1)
        else:
            a_h, i_h1, i_h2, i_h3 = None, None, None, None

        # a_rep = torch.cat((a_h[0, :, :], a_h[-1, :, :]), dim=-1)
        # i_rep = torch.cat((i_h[0, :, :], i_h[-1, :, :]), dim=-1)
        a_rep = a_h[-1, :, :]
        i_rep = torch.cat((i_h1[-1, :, :], i_h2[-1, :, :], i_h3[-1, :, :]), dim=0)
    else:
        q_h = t5_model(inputs_embeds=fc(q_e)).last_hidden_state.transpose(0, 1)
        q_rep = torch.cat((q_h[0, :, :], q_h[-1, :, :]), dim=-1)
        a_h = t5_model(inputs_embeds=fc(a_e)).last_hidden_state.transpose(0, 1)
        a_rep = torch.cat((a_h[0, :, :], a_h[-1, :, :]), dim=-1)
        i = i.reshape(i.shape[0], i.shape[1], -1, 1, i.shape[4], i.shape[5])
        i_h = t5_model(inputs_embeds=fc(to_pytorch(flatten_qail(i)))).last_hidden_state.transpose(0, 1)
        i_rep = torch.cat((i_h[0, :, :], i_h[-1, :, :]), dim=-1)

        # transcript representation
        t_full = t5_model(inputs_embeds=fc(t_e)).last_hidden_state.transpose(0, 1)
        # visual representation
        v_full = t5_model(inputs_embeds=v_fc(v_e)).last_hidden_state.transpose(0, 1)
        ...
        # ac_seq = ac_full[0]
        #
        t_rep = torch.cat((t_full[0, :, :], t_full[-1, :, :]), dim=-1)
        v_rep = torch.cat((v_full[0, :, :], v_full[-1, :, :]), dim=-1)
    ...
    for i in range(100):
        ...
        for j in range(iters):
            ...
            q_rep, a_rep, i_rep, t_rep, v_rep = \
                feed_forward(this_trk, t5_model, fc, q_lstm, a_lstm, v_lstm, t_lstm,
                             ac_lstm, mfn_mem, mfn_delta1, mfn_delta2, mfn_tfn,
                             preloaded_train, perturbed_train, v_fc,
                             context_matters=config['context_matter'], load_data=load_data,
                             offset=config['perturbation_offset'])

            if load_data == 'a' or config['context_matter']:
                correct = judge(a_rep)
                incorrect_1 = judge(i_rep[0:a_rep.shape[0], :])
                incorrect_2 = judge(i_rep[a_rep.shape[0]:2*a_rep.shape[0], :])
                incorrect_3 = judge(i_rep[2*a_rep.shape[0]:, :])
            elif load_data == 'qa':
                correct = judge(torch.cat((q_rep, a_rep), 1))
                incorrect_1 = judge(torch.cat((q_rep, i_rep[0:a_rep.shape[0], :]), 1))
                incorrect_2 = judge(torch.cat((q_rep, i_rep[a_rep.shape[0]:2 * a_rep.shape[0], :]), 1))
                incorrect_3 = judge(torch.cat((q_rep, i_rep[2*a_rep.shape[0]:, :]), 1))
            elif load_data == 'qat':
                correct = judge(torch.cat((q_rep, a_rep, t_rep), 1))
                incorrect_1 = judge(torch.cat((q_rep, i_rep[0:a_rep.shape[0], :], t_rep), 1))
                incorrect_2 = judge(torch.cat((q_rep, i_rep[a_rep.shape[0]:2 * a_rep.shape[0], :], t_rep), 1))
                incorrect_3 = judge(torch.cat((q_rep, i_rep[2 * a_rep.shape[0]:, :], t_rep), 1))
            elif load_data == 'qav':
                correct = judge(torch.cat((q_rep, a_rep, v_rep), 1))
                incorrect_1 = judge(torch.cat((q_rep, i_rep[0:a_rep.shape[0], :], v_rep), 1))
                incorrect_2 = judge(torch.cat((q_rep, i_rep[a_rep.shape[0]:2 * a_rep.shape[0], :], v_rep), 1))
                incorrect_3 = judge(torch.cat((q_rep, i_rep[2 * a_rep.shape[0]:, :], v_rep), 1))
            # elif load_data == 'qaac':
            ...
            incorrect_mean_1 = Variable(torch.Tensor(numpy.array([0.])), requires_grad=False).cuda()
            incorrect_mean_2 = Variable(torch.Tensor(numpy.array([0.])), requires_grad=False).cuda()
            incorrect_mean_3 = Variable(torch.Tensor(numpy.array([0.])), requires_grad=False).cuda()

            optimizer.zero_grad()
            loss = (nn.MSELoss()(correct.mean(dim=0), correct_mean) + nn.MSELoss()(incorrect_1.mean(dim=0),
                                                                                   incorrect_mean_1) +
                    nn.MSELoss()(incorrect_2.mean(dim=0), incorrect_mean_2) + nn.MSELoss()(incorrect_3.mean(dim=0),
                                                                                           incorrect_mean_3))
            loss.backward()
            optimizer.step()
            losses.append(loss.cpu().detach().numpy())
            accs.append(calc_accuracy(correct, incorrect_1, incorrect_2, incorrect_3))
        ...
        for j in range(iters):
            ...
            _accs.append(calc_accuracy(correct, incorrect_1, incorrect_2, incorrect_3))
