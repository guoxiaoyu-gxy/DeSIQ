...
if __name__ == '__main__':
    ...
    load_data = 'a'
    noise = 'replace_correct_with_incorrect'
    bs = 4
    ...
    t5_model, fc, q_lstm, a_lstm, t_lstm, v_lstm, ac_lstm, \
        mfn_mem, mfn_delta1, mfn_delta2, mfn_tfn = init_tensor_mfn_modules('best-multi.pt')
    ...
    for j in range(iters):
        ...
        if load_data == 'a':
            correct = judge(a_rep)
            incorrect_1=judge(i_rep[0:q_rep.shape[0],:])
            incorrect_2=judge(i_rep[q_rep.shape[0]:2*q_rep.shape[0],:])
            incorrect_3=judge(i_rep[2*q_rep.shape[0]:,:])
        if load_data == 'qa':
            correct = judge(torch.cat((q_rep, a_rep), 1))
            incorrect_1=judge(torch.cat((q_rep,i_rep[0:q_rep.shape[0],:]),1))
            incorrect_2=judge(torch.cat((q_rep,i_rep[q_rep.shape[0]:2*q_rep.shape[0],:]),1))
            incorrect_3=judge(torch.cat((q_rep,i_rep[2*q_rep.shape[0]:,:]),1))
        # elif load_data == 'qat':
        ...
        if noise == 'replace_incorrect_with_incorrect':
            rand_index = torch.randperm(len(incorrect_1))
            incorrect_1 = incorrect_1[rand_index]
            rand_index = torch.randperm(len(incorrect_2))
            incorrect_2 = incorrect_2[rand_index]
            rand_index = torch.randperm(len(incorrect_3))
            incorrect_3 = incorrect_3[rand_index]
        elif noise == 'replace_incorrect_with_correct':
            rand_index = torch.randperm(len(correct))
            incorrect_1 = correct[rand_index]
            rand_index = torch.randperm(len(correct))
            incorrect_2 = correct[rand_index]
            rand_index = torch.randperm(len(correct))
            incorrect_3 = correct[rand_index]
        elif noise == 'replace_correct_with_correct':
            rand_index = torch.randperm(len(correct))
            correct = correct[rand_index]
        elif noise == 'replace_correct_with_incorrect':
            rand_index = torch.randperm(len(incorrect_1))
            correct = incorrect_1[rand_index]
